...
BMIQ <- function(beta.v,design.v,nL=3,doH=TRUE,nfit=10000,th1.v=c(0.2,0.75),th2.v=NULL,niter=5,tol=0.001,plots=FALSE,sampleID=1){
...
sel.idx <- which(d1.o$x < 0.4);
mod1U <- d1.o$x[sel.idx[which.max(d1.o$y[sel.idx])]];
sel.idx <- which(d1.o$x > 0.5);
mod1M <- d1.o$x[sel.idx[which.max(d1.o$y[sel.idx])]];
...
sel.idx <- which(d2.o$x < 0.4);
mod2U <- d2.o$x[sel.idx[which.max(d2.o$y[sel.idx])]]
sel.idx <- which(d2.o$x > 0.5);
mod2M <- d2.o$x[sel.idx[which.max(d2.o$y[sel.idx])]]
