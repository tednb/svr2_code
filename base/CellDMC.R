...
CellDMC <- function(beta.m, pheno.v, frac.m, 
  ...
  print("start")
  # Parallel processing
  results <- mclapply(seq_len(nrow(beta.m)), function(i) {
    beta.v <- beta.m[i, ]
    ...
    # Return a list containing the formula and the coefficients
    list("formula" = summary(Int.o), "coefficients" = IntCoe.v)
  }, mc.cores = mc.cores)
  
  # Post-processing to separate formulas and coefficients
  print("formulas...")
  formulas <- mclapply(results, function(x) x$formula, mc.cores = mc.cores)
  allCoe.m <- do.call(rbind, lapply(results, function(x) x$coefficients))
  ...
  return(list(dmct = dmct.m, coe = coe.ld, equ = formulas))
}
